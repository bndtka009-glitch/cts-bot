...
class CTSDatabase:
    ...
    def init_tables(self):
        self.cursor.execute('''CREATE TABLE IF NOT EXISTS users (
            ...
            role TEXT DEFAULT 'passenger', emp_id TEXT,
            points INTEGER DEFAULT 0, banned INTEGER DEFAULT 0)''')
        self.cursor.execute('''CREATE TABLE IF NOT EXISTS reports (
            id INTEGER PRIMARY KEY AUTOINCREMENT, driver_id INTEGER NOT NULL,
            driver_name TEXT, location TEXT, media_id TEXT, media_type TEXT,
            description TEXT, drone_req INTEGER DEFAULT 0,
        ...
        for table, col, definition in [
            ...
            ("reports","driver_name","TEXT"), ("reports","media_type","TEXT"),
            ("reports","description","TEXT"), ("reports","drone_req","INTEGER DEFAULT 0"),
        ]:
            try:
    ...
    def register_user(self, user_id, username, full_name):
        self.cursor.execute(
            "INSERT OR IGNORE INTO users (user_id,username,full_name,role) VALUES(?,?,?,'passenger')",
    ...
    def upsert_admin(self, user_id):
        self.cursor.execute("INSERT OR IGNORE INTO users (user_id,role) VALUES(?,'admin')",(user_id,))
        self.cursor.execute("UPDATE users SET role='admin' WHERE user_id=?",(user_id,))
        self.conn.commit()
    ...
    def search_users(self, query):
        ...
        return self.cursor.execute(
            "SELECT user_id,username,full_name,role,emp_id,points,banned FROM users "
            "WHERE user_id!=? AND (CAST(user_id AS TEXT) LIKE ? OR username LIKE ? "
            "OR full_name LIKE ? OR emp_id LIKE ?) ORDER BY role,full_name LIMIT 20",
            (SUPER_ADMIN_ID,q,q,q,q)).fetchall()

    def get_all_users(self, page=0, per_page=10):
        offset = page * per_page
        total  = self.cursor.execute(
            "SELECT COUNT(*) FROM users WHERE user_id!=?",(SUPER_ADMIN_ID,)).fetchone()[0]
        users  = self.cursor.execute(
            "SELECT user_id,username,full_name,role,emp_id,points,banned FROM users "
            "WHERE user_id!=? ORDER BY role,full_name LIMIT ? OFFSET ?",
    ...
    def get_driver_reports(self, driver_id):
        return self.cursor.execute(
    ...
    def reject_report(self, report_id, reason):
        self.cursor.execute(
            "UPDATE reports SET status='rejected',reject_reason=? WHERE id=?",(reason,report_id))
        res = self.cursor.execute(
            "SELECT driver_id FROM reports WHERE id=?",(report_id,)).fetchone()
        self.conn.commit()
        return res[0] if res else None
...
# Добавление репорта админом — те же 2 шага + описание
class AdminAddReport(StatesGroup):
    description = State()   # шаг 1: описание
    media       = State()   # шаг 2: фото/видео
    location    = State()   # шаг 3: геолокация
    status      = State()   # шаг 4: статус
...
# ХЕЛПЕРЫ
# ==========================================
ROLE_ICON   = {"driver":"🚌","passenger":"👤","admin":"👑"}
...
def kb_report_actions(report_id):
...
def kb_report_manage(report_id):
    return InlineKeyboardMarkup(inline_keyboard=[[
...
def kb_user_actions(user_id, banned, role):
...
def kb_pagination(page, total, per_page=10):
    total_pages = max(1,(total+per_page-1)//per_page)
    row = []
    if page > 0:
        row.append(InlineKeyboardButton(text="◀️", callback_data=f"upage_{page-1}"))
    row.append(InlineKeyboardButton(text=f"{page+1}/{total_pages}", callback_data="upage_noop"))
    if (page+1)*per_page < total:
        row.append(InlineKeyboardButton(text="▶️", callback_data=f"upage_{page+1}"))
    return InlineKeyboardMarkup(inline_keyboard=[row])

def kb_report_status():
    return InlineKeyboardMarkup(inline_keyboard=[[
...
async def cmd_start(message: types.Message):
    ...
    if user:
        ...
        if banned:
            ...
            return
        kb = {"driver": kb_driver, "admin": kb_admin}.get(role, kb_passenger)()
        role_label = {"driver":"🚌 Водитель","admin":"👑 Админ","passenger":"👤 Пассажир"}.get(role,role)
        await message.answer(
...
async def user_console_enter(message: types.Message, state: FSMContext):
    ...
    await _send_console(message, users, total, page=0)

async def _send_console(message: types.Message, users, total, page):
    per_page = 10
    header = (
        f"🖥 *КОНСОЛЬ ПОЛЬЗОВАТЕЛЕЙ*\n"
        f"Всего: *{total}* | Стр. *{page+1}*\n\n"
        f"🔍 Введите ID, @username или имя для поиска\n"
        f"{'─'*22}"
    )
    if not users:
        await message.answer(header + "\n\n📭 Пусто.", reply_markup=kb_console_exit(),
                             parse_mode="Markdown")
        return
    lines = [header]
    for u in users:
        ...
        ustr     = f"@{username}" if username else "—"
        lines.append(f"\n{icon}{ban_mark} *{full_name or 'Без имени'}*\n"
                     f"   🆔 `{uid}` | {ustr} | ⭐{points}")
    await message.answer("\n".join(lines), reply_markup=kb_console_exit(),
                         parse_mode="Markdown")
    if total > per_page:
        await message.answer("📄 Страницы:", reply_markup=kb_pagination(page, total))
...
async def user_console_search(message: types.Message, state: FSMContext):
    ...
    if not results:
        ...
        return
    await message.answer(
        f"🔍 Найдено *{len(results)}* по запросу *«{query}»*:",
        parse_mode="Markdown")
    for u in results:
        await message.answer(
            format_user_card(u),
...
async def users_paginate(callback: types.CallbackQuery):
    ...
    await _send_console(callback.message, users, total, page=page)
    await callback.answer()

@dp.callback_query(F.data.startswith("urep_"))
async def user_reports_console(callback: types.CallbackQuery):
    uid     = int(callback.data.split("urep_")[1])
    reports = db.get_driver_reports(uid)
    user    = db.get_user(uid)
    name    = user[2] if user else str(uid)
    if not reports:
        await callback.answer("Нет репортов.", show_alert=True)
        return
    lines = [f"📜 *Репорты — {name}:*\n"]
    for r in reports:
...
async def adjust_points_start(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer(
        ...
        f"Баланс: *{pts}* баллов\n\n"
        f"Введите число (например `+5` или `-3`):",
        parse_mode="Markdown")
    await callback.answer()
...
async def adjust_points_finish(message: types.Message, state: FSMContext):
    ...
    await message.answer(
        f"✅ *{name}*: {sign}{delta} → *{pts}* баллов.",
        reply_markup=kb_admin(), parse_mode="Markdown")
    try:
        await bot.send_message(uid,
            f"⭐ Администратор {'начислил' if delta>=0 else 'снял'} *{abs(delta)}* балл(ов).\n"
            f"Ваш баланс: *{pts}* баллов.", parse_mode="Markdown")
    except: pass

# ==========================================
# ВОДИТЕЛЬ — РЕПОРТ (шаг 1: медиа, шаг 2: геолокация)
# ==========================================
async def start_report_flow(message: types.Message, state: FSMContext, drone_req=0):
    ...
    await message.answer(
        ...
        f"Можно добавить описание в подписи.",
...
async def cancel_any(message: types.Message, state: FSMContext):
    ...
    if user:
        role = user[3]
        kb   = kb_admin() if role=='admin' else kb_driver() if role=='driver' else kb_passenger()
    else:
        kb = kb_passenger()
...
async def media_wrong(message: types.Message):
    await message.answer("⚠️ Отправьте *фото* или *видео*.", parse_mode="Markdown")
...
async def process_location(message: types.Message, state: FSMContext):
    ...
    flag    = "🚁 *ДРОН-АУДИТ*\n\n" if drone_req else ""
    caption = (f"{flag}📦 Репорт *#{report_id}*\n"
               f"👤 {driver_name} (`{message.from_user.id}`)\n"
               f"📍 [{location}]({maps_link})\n"
               f"📝 {data.get('description') or '—'}")
    if data['media_type'] == "photo":
        await bot.send_photo(SUPER_ADMIN_ID, photo=data['media_id'], caption=caption,
                             reply_markup=kb_report_actions(report_id), parse_mode="Markdown")
    else:
        await bot.send_video(SUPER_ADMIN_ID, video=data['media_id'], caption=caption,
                             reply_markup=kb_report_actions(report_id), parse_mode="Markdown")
...
async def location_wrong(message: types.Message):
    await message.answer(
        "⚠️ Нажмите *«📍 Отправить геолокацию»* ниже.",
        reply_markup=kb_send_location(), parse_mode="Markdown")
...
async def check_bonus(message: types.Message):
    ...
    left = 4 - (pts % 4)
    text = f"⭐ Баллов: *{pts}*\n📅 Дней отдыха: *{days}*\n\n"
    text += f"До следующего дня: *{left}* репорта." if left < 4 else "4 балла = 1 день отгула!"
...
async def show_pending_reports(message: types.Message):
    ...
    if not pending:
        ...
        return
    await message.answer(f"📋 Ожидают: *{len(pending)}*", parse_mode="Markdown")
    for r in pending:
        ...
        caption   = (f"{drone}📦 Репорт *#{rid}*\n"
                     f"👤 {driver_name} (`{driver_id}`)\n"
                     f"📍 [{location}]({maps_link})\n"
                     f"📝 {desc or '—'}")
        try:
            if media_type == "video":
                await bot.send_video(message.chat.id, video=media_id, caption=caption,
                                     reply_markup=kb_report_actions(rid), parse_mode="Markdown")
            else:
                await bot.send_photo(message.chat.id, photo=media_id, caption=caption,
                                     reply_markup=kb_report_actions(rid), parse_mode="Markdown")
        except:
...
async def admin_reject_finish(message: types.Message, state: FSMContext):
    ...
    await state.clear()
    await message.answer(f"📋 Репорт *#{r_id}* отклонён. _{reason}_", parse_mode="Markdown")
    if driver_id:
        await bot.send_message(driver_id,
...
async def admin_manage_reports(message: types.Message):
    ...
    if not reports:
        ...
        return
    await message.answer("🛠 *Последние 15 репортов:*", parse_mode="Markdown")
    for r in reports:
...
# ==========================================
# АДМИН — ДОБАВИТЬ РЕПОРТ (3 шага: описание → медиа → геолокация → статус)
# ==========================================
...
async def admin_add_start(message: types.Message, state: FSMContext):
    ...
    await message.answer(
        ...
        "*Шаг 1 из 3* — Введите описание инцидента:",
        reply_markup=kb_cancel(), parse_mode="Markdown")
...
async def admin_add_description(message: types.Message, state: FSMContext):
    ...
    await message.answer(
        "*Шаг 2 из 3* — Отправьте 📸 фото или 🎥 видео инцидента.\n"
...
async def admin_add_media(message: types.Message, state: FSMContext):
    ...
    await message.answer(
        "*Шаг 3 из 3* — Отправьте 📍 геолокацию.\n"
        "Нажмите кнопку ниже или напишите координаты вручную (51.1283, 71.4305).",
        reply_markup=kb_send_location(), parse_mode="Markdown")
...
async def admin_add_media_skip(message: types.Message, state: FSMContext):
    ...
    await message.answer(
        "*Шаг 3 из 3* — Отправьте 📍 геолокацию.\n"
        "Нажмите кнопку ниже или введите координаты вручную.",
        reply_markup=kb_send_location(), parse_mode="Markdown")
    await state.set_state(AdminAddReport.location)
...
@dp.message(AdminAddReport.location, F.location)
async def admin_add_location_geo(message: types.Message, state: FSMContext):
    lat, lon  = message.location.latitude, message.location.longitude
    location  = f"{lat}, {lon}"
    maps_link = f"https://maps.google.com/?q={lat},{lon}"
    await state.update_data(location=location, maps_link=maps_link)
    await message.answer(
        f"📍 Геолокация получена: `{location}`\n\n"
        f"📌 Выберите статус репорта:",
        reply_markup=kb_report_status(), parse_mode="Markdown")
    await state.set_state(AdminAddReport.status)

# Геолокация вручную (текстом)
@dp.message(AdminAddReport.location, F.text)
async def admin_add_location_text(message: types.Message, state: FSMContext):
...
async def admin_edit_location(message: types.Message, state: FSMContext):
    ...
    await message.answer("📝 Новое описание (или `skip`):")
...
async def set_driver(callback: types.CallbackQuery):
    ...
    name = user[2] if user else str(uid)
    await callback.message.edit_text(
        f"🚌 *{name}* (`{uid}`) — *водитель*.", parse_mode="Markdown")
...
async def set_passenger(callback: types.CallbackQuery):
    ...
    name = user[2] if user else str(uid)
    await callback.message.edit_text(
        f"👤 *{name}* (`{uid}`) — *пассажир*.", parse_mode="Markdown")
    await callback.answer("✅ Роль изменена")
    try:
        await bot.send_message(uid,
...
async def admin_ban(callback: types.CallbackQuery):
    ...
    db.set_banned(uid, 1)
    await callback.message.edit_text(
        f"🚫 `{uid}` заблокирован.", parse_mode="Markdown")
...
async def admin_unban(callback: types.CallbackQuery):
    ...
    db.set_banned(uid, 0)
    await callback.message.edit_text(
        f"✅ `{uid}` разблокирован.", parse_mode="Markdown")
